...
from app.ai_compliance import check_policy_compliance
...
@app.get(
    "/api/v1/citizen/{user_id}/requests",
    response_model=list[ConsentLog],
    tags=["Citizen (Ayo)"],
)
async def get_pending_requests(user_id: str):
    requests = consent_log_collection.find({"user_id": user_id, "status": "pending"}).sort(
        "timestamp", -1
    )
...
async def request_data_access(body: DataRequestBody, org: Organization = Depends(get_current_org)):
    ...
    is_compliant = await check_policy_compliance(
        policy_text=org.policy_text, data_type=body.data_type, purpose=body.purpose
    )
...
if __name__ == "__main__":
    # If someone runs app/main.py directly, tell uvicorn to load the app as a package.
